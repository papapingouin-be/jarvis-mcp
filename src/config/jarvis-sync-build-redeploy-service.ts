...
async function resolveConfig(envFile?: string): Promise<ResolvedConfig> {
  ...
  return {
    ...
    webhookUrl,
    mcpoContainerName: firstNonEmpty(mergedEnv, "JARVIS_MCPO_CONTAINER_NAME") ?? "",
    portainerUseStackWebhook: parseBoolean(firstNonEmpty(mergedEnv, "PORTAINER_USE_STACK_WEBHOOK"), true),
    restartStrategy: (firstNonEmpty(mergedEnv, "RESTART_STRATEGY") ?? "webhook") as ResolvedConfig["restartStrategy"],
