...
import { defaultFetchRunner, type FetchRunner } from "../../executors/http/fetch-runner.js";
import {
  createExecFileCommandRunner,
  type CommandResult,
  type CommandRunner,
} from "../../executors/process/command-runner.js";
...
const defaultCommandRunner = createExecFileCommandRunner({
  defaultTimeoutMs: EXECUTION_TIMEOUT_MS,
});
...
async function resolveConfig(envFile?: string): Promise<ResolvedConfig> {
  ...
  return {
    ...
    webCodeLocalSubdir: firstNonEmpty(mergedEnv, "WEB_CODE_LOCAL_SUBDIR") ?? "apps/config-web",
...
export class JarvisSyncBuildRedeployService {
  constructor(
    ...
    private readonly fetchRunner: FetchRunner = defaultFetchRunner,
  ) {}
